...
//		cout << "clock: " << clock << endl;
		...
		if (isCustomerArrived(timeBetweenCustomerArrival))
		{
			...
			newCustomer.setCustomerInfo(custNumber, clock, 0, transactionTime);
			customerQueue.push(newCustomer);
			custNumber++;

		}

//		cout << "num busy servers" << serverList. getNumberOfBusyServers() << endl;
		serverID = serverList.getFreeServerID();
		if (serverID != -1 && !customerQueue.empty())
		{
			/**************
			 * TODO: Implement what happens when a server is free
			 * server gets and seats a customer
			 * takes the customer out of the queue
			 **************/
			serverList.setServerBusy(serverID, customerQueue.front(), clock);
			totalWaitTime +=  customerQueue.front().getWaitingTime();
			// you need the front to make the queue into a customer type to get the wait time
			customerQueue.pop();
